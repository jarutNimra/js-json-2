(function(){const l=document.createElement("link").relList;if(l&&l.supports&&l.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))r(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const i of t.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function a(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function r(e){if(e.ep)return;e.ep=!0;const t=a(e);fetch(e.href,t)}})();const o=async()=>{try{let a=(await(await fetch("/db.json")).json()).menu.map(r=>r.dropdown.length>0?`<li>
            ...
            </li>`:` <li><a href=${r.link} class='capitalize'>${r.value}</a></li>`);document.querySelector(".menu-desktop").innerHTML=a.join(" ")}catch(s){console.log(s)}},c=async()=>{try{let a=(await(await fetch("/db.json")).json()).slider.map(r=>`<swiper-slide class="relative">
            <div class="h-full">
              <a href=${r.url}>
                <img
                src=${r.image}
          ...
          </swiper-slide>`);document.querySelector(".mySwiper").innerHTML=a.join(" ")}catch(s){console.log(s)}},n=async()=>{try{let a=(await(await fetch("/db.json")).json()).products.map(r=>` <div class="card bg-base-100 w-96 shadow-sm">
            <figure>
              <img
                src=${r.image}
                alt="Shoes" />
